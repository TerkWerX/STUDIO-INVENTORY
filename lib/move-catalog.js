...
const { isEncryptedBackupFile } = require('./backup-crypto');
const { readZipEntry } = require('./folder-backup');

const PENDING_NAME = 'pending-move-restore.zip';

const { isLocalRequest: isLocalAddress, hostGuard, crossSiteGuard } = require('./request-guard');
...
async function assertPlainRecoveryZip(filePath) {
...
function listen({ port, message, dbPath, dataDir }) {
  const app = express();
  const upload = multer({
    dest: path.join(dataDir, 'backups', '.incoming'),
  ...
  app.post('/api/move/zip', upload.single('backup'), async (req, res) => {
    if (!req.file?.path) return res.status(400).json({ error: 'Choose the recovery ZIP' });
    try {
      await assertPlainRecoveryZip(req.file.path);
      const pending = path.join(dataDir, PENDING_NAME);
      await fs.promises.copyFile(req.file.path, pending);
