...
- **Two devices can arrange the studio at once.** Adding or removing gear in a rack or signal chain, and placing, moving or removing gear on a room map, now changes just that one entry. Before, each change re-sent the whole list as that device last saw it, so a phone with the map open could quietly undo what someone else had just done on the studio computer. Placing gear from the map page also no longer resets the wall-photo alignment of other gear in the room.
