import {
  formatCurrency, formatDate, escapeHtml, safeUrl, fileUrl, brandLogoHtml, isDriverCategory,
      ...
      <div class="filter-group">
        ...
        <div class="form-group">
          ...
          <select id="filter-condition">
            ...
            ${meta.conditions.map(c => `<option value="${escapeHtml(c)}" ${f.condition === c ? 'selected' : ''}>${escapeHtml(c)}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          ...
          <tbody>
            ${items.map(item => `
              <tr data-action="view-item" data-id="${item.id}" class="${item.parent_item_id ? 'accessory-row' : ''}">
                ...
                <td><span class="condition-badge condition-${escapeHtml(item.condition)}">${escapeHtml(item.condition)}</span></td>
  ...
  return `
    ...
    <div class="card identity-card">
      ...
      <div class="detail-grid">
        ...
        <div class="detail-field"><div class="field-label">Condition</div><div class="field-value"><span class="condition-badge condition-${escapeHtml(item.condition)}">${escapeHtml(item.condition)}</span></div></div>
          ...
          ${software.map(s => `
            <div class="software-item">
              <div class="software-meta">
                ...
                <div class="text-muted-sm">Archived ${formatDate(s.created_at?.split(' ')[0])}${s.source_url ? ` · <a href="${escapeHtml(safeUrl(s.source_url))}" target="_blank" rel="noopener">Source</a>` : ''}</div>
              </div>
              <div class="btn-group">
...
async function uploadPhotoFiles(zone, itemId, files, { onUpload, onError }) {
  ...
  // One upload at a time per drop zone: a second paste or drop while the first
  // is still sending would upload the same photos twice.
  if (zone.classList.contains('photo-drop-uploading')) {
    onError?.('Still uploading the previous photos — try again in a moment.');
    return;
  }
