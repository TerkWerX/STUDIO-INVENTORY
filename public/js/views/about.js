...
export function renderAbout() {
  return `
    ...
    <div class="card">
      <h3 class="section-title">Server Log</h3>
      <p style="color:var(--text-secondary);line-height:1.8">
        Studio Inventory writes what happens on the studio computer (start-ups, backups, errors) to a log file.
        When something goes wrong, the details are here. Share links and PINs are hidden in the log.
      </p>
      <p id="server-log-path" style="color:var(--text-muted);word-break:break-all"></p>
      <div class="btn-group" style="margin-top:0.75rem">
        <button type="button" class="btn btn-secondary" id="server-log-show">Show Recent Log</button>
        <button type="button" class="btn btn-secondary" id="server-log-open">Open Log Folder</button>
      </div>
      <pre id="server-log-lines" class="hidden" style="margin-top:0.75rem;max-height:24rem;overflow:auto;white-space:pre-wrap;word-break:break-word;background:var(--bg-tertiary);padding:0.75rem;border-radius:6px;font-size:0.8rem"></pre>
    </div>
  `;
}
